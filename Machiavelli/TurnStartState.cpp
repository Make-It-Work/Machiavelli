...
{
	game = gameHandler;
	const Character& chr = gameHandler->getCharacterHandler().getCharacterRef(gameHandler->getTurnID());
...
void TurnStartState::print(std::shared_ptr<Player> player)
{
	...
	player->get_socket()->write("Je bent nu de: " + game->getCharacterHandler().getCharacterRef(game->getTurnID()).getName() + "\r\n");
	...
	player->get_socket()->write("[3] Maak gebruik van de karaktereigenschap van de " + game->getCharacterHandler().getCharacterRef(game->getTurnID()).getName() + " \r\n");
}
